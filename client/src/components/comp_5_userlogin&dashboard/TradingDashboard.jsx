...
const TradingDashboard = () => {
  ...
  const [stockList, setStockList] = useState(["MSFT", "AAPL", "GOOG", "TSLA"]); // Example, replace with backend if available
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get(`/api/market/stock/${selectedStock}`),
      api.get(`/api/market/stock/${selectedStock}/trades`),
    ])
      .then(([userRes, appsRes, stockRes, tradesRes]) => {
        setUserInfo(userRes.data.data.user);
        const applications = appsRes.data.data.applications || [];
        const totalInvested = applications.reduce(
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="bg-gray-800 border-b border-gray-700">
        <div className="flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-6 ml-8">
              <div>
                ...
                <div className="font-semibold">
                  ₹{portfolioStats.totalInvested?.toLocaleString() || 0}
                </div>
              </div>
              <div>
                <div className="text-xs text-gray-400">Total Applications</div>
                <div className="font-semibold">
                  {portfolioStats.totalApplications}
                </div>
              </div>
            </div>
            ...
            <div className="ml-8">
              <label className="text-xs text-gray-400 mr-2">Stock:</label>
              <select
                value={selectedStock}
      ...
      {loading ? (
        <div className="flex-1 flex items-center justify-center text-cyan-400 text-xl">
          Loading dashboard...
        </div>
      ) : error ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-700 rounded-lg p-3 space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>
                      ${portfolioStats.totalInvested?.toFixed(2) || 0}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
